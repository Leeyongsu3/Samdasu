...
export default class VehicleController extends ZepetoScriptBehaviour {
    ...
    public vehicleType:VehicleType = VehicleType.Horse;
    ...
    private rotToAccel(value:rotationData, speed:number, mass:number) {
     ...
     private ControllerSet() {
        ...
        if(this.vehicleType == VehicleType.Horse) {
            ...
            const joyConD = controller.transform.GetChild(0).GetChild(3).GetChild(2).gameObject.GetComponent<JoystickController>();
            const joyConJ = controller.transform.GetChild(0).GetChild(4).gameObject.GetComponent<JoystickController>();
            if(joyConD) joyConD.target = this;
            if(joyConJ) joyConJ.target = this;

        } else if(this.vehicleType == VehicleType.Wheel) {
        ...
        } else if(this.vehicleType == VehicleType.Merry_Go_Round) {
            // controller.transform.GetChild(0).GetChild(3).gameObject.SetActive(true);
            // controller.transform.GetChild(0).GetChild(4).gameObject.SetActive(true);
