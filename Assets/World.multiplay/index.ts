...
import {Player, sVector3, sQuaternion, SyncTransform, PlayerAdditionalValue, ZepetoAnimationParam, EquipData, Sticker } from "ZEPETO.Multiplay.Schema";
...
export default class extends Sandbox {
    ...
    onCreate(options: SandboxOptions) {
        ...
        /** Samdasu **/
        this.onMessage(MESSAGE.Pick_Trash, (client, trash) => {
            if(!trash.Pick_Trash) return;
            const player = this.state.players.get(client.sessionId);
            player.samdasu.TrashCount += 1;
            client.send(MESSAGE.Pick_Trash, null);
        });
        
        this.onMessage(MESSAGE.Add_Sticker, (client, message) => {
            const player = this.state.players.get(client.sessionId);
            if(player.samdasu.TrashCount < 10) {
                client.send(MESSAGE.Add_Sticker, false);
            } else {
                player.samdasu.TrashCount -= 10;
                
                client.send(MESSAGE.Add_Sticker, true);
                client.send(MESSAGE.Pick_Trash, null);
            }
        });
        
        this.onMessage(MESSAGE.Add_Point, (client, message) => {
            const player = this.state.players.get(client.sessionId);
    ...
    async onJoin(client: SandboxPlayer) {
        ...
        /* Trash Sticker */
        // let stickers = await storage.get("Stickers") as ArraySchema<Sticker>;
        // if (stickers == null) stickers = this.StickerInit();
        // console.log(`[OnJoin] ${client.sessionId}'s stickers : ${stickers}`);
        // player.samdasu.Stickers = this.StickerInit();
        console.log(`[OnJoin] ${client.sessionId}'s stickers : ${player.samdasu.Stickers}`);
        // for(const sticker of stickers) {
        //     player.samdasu.Stickers.push(sticker);
        // }
        await storage.set("Stickers", player.samdasu.Stickers);
    ...
    async onLeave(client: SandboxPlayer, consented?: boolean) {
        ...
        /* Trash Sticker */
        console.log(`[onLeave] ${client.sessionId}'s stickers : ${player.samdasu.Stickers}`)
        await client.loadDataStorage().set("Stickers", player.samdasu.Stickers);
    ...
    StickerInit() {
        const stickers = [];
        const stickers_name:Array<string> = [
            "01", "02", "03", "04", "05",
            "06", "07", "08", "09", "10",
        ];
        if(stickers_name) return null;
        for(const name of stickers_name) {
            const sticker = new Sticker();
            sticker.name = name;
            sticker.count = 0;
            stickers.push(sticker);
        }
        return stickers as ArraySchema<Sticker>;
    }
